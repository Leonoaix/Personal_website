...
import StaticRouters from './modules/StaticRouters';
import { useSelector } from 'react-redux';
import { RootState } from '../store';  // Import RootState from your actual store file
import { useEffect } from 'react';
import Loading from '../pages/Loading';
...
const AppRouter = () => {
  ...
  const token = true; // This should be derived from your actual authentication logic
  
  useEffect(() => {
    if (!token) {
      navigate('/login');  // Navigate to login if no token
    }
  }, [token, navigate]);  // Adding dependencies to avoid unnecessary re-renders

  // Display the loading screen if still loading, otherwise show the app
  return (
    <>
      {loading ? <Loading /> : <StaticRouters />}
    </>
  );
};
